...
@interface IAPHandlerDelegate : NSObject <SKRequestDelegate,
...
- (void)productsRequest:(nonnull SKProductsRequest*)request
  ...
  if (response.invalidProductIdentifiers) {
    ...
    for (unsigned long i = 0, count = [products count]; i < count; ++i) {
      ...
      QMetaObject::invokeMethod(
          m_handler, "unknownProductRegistered", Qt::QueuedConnection,
  ...
  if (products) {
    ...
    for (unsigned long i = 0, count = [products count]; i < count; ++i) {
      ...
      QMetaObject::invokeMethod(m_handler, "productRegistered",
                                Qt::QueuedConnection, Q_ARG(void*, product));
    }
  }

  QMetaObject::invokeMethod(m_handler, "productsRegistrationCompleted",
                            Qt::QueuedConnection);
...
- (void)paymentQueue:(nonnull SKPaymentQueue*)queue
      ...
      case SKPaymentTransactionStatePurchased: {
        ...
        if (transaction.transactionState == SKPaymentTransactionStateRestored) {
          ...
          if (originalTransaction) {
            QString originalIdentifier = QString::fromNSString(
                originalTransaction.transactionIdentifier);
            QDateTime originalDate =
                QDateTime::fromNSDate(originalTransaction.transactionDate);
            logger.log() << "original transaction identifier: "
                         << originalIdentifier
        ...
        if (settingsHolder->hasSubscriptionTransaction(identifier)) {
          logger.log() << "This transaction has already been processed. Let's "
                          "ignore it.";
        } else {
  ...
  if (!completedTransactions && !canceledTransactions && !failedTransactions) {
    // Nothing completed, nothing restored, nothing failed. Just purchasing
    // transactions.
    return;
  }
  ...
  if (canceledTransactions) {
    ...
    QMetaObject::invokeMethod(m_handler, "stopSubscription",
                              Qt::QueuedConnection);
    QMetaObject::invokeMethod(m_handler, "subscriptionCanceled",
                              Qt::QueuedConnection);
    ...
    logger.log()
        << "Subscription completed - but all the transactions are known";
    QMetaObject::invokeMethod(m_handler, "stopSubscription",
                              Qt::QueuedConnection);
    QMetaObject::invokeMethod(m_handler, "subscriptionCanceled",
                              Qt::QueuedConnection);
    ...
    QMetaObject::invokeMethod(m_handler, "processCompletedTransactions",
                              Qt::QueuedConnection,
                              Q_ARG(QStringList, completedTransactionIds));
  } else {
    Q_ASSERT(completedTransactions);
    logger.log()
        << "Subscription completed - but the user is not authenticated yet";
    QMetaObject::invokeMethod(m_handler, "stopSubscription",
                              Qt::QueuedConnection);
    QMetaObject::invokeMethod(m_handler, "subscriptionCanceled",
                              Qt::QueuedConnection);
...
- (void)request:(SKRequest*)request didFailWithError:(NSError*)error {
  ...
  QMetaObject::invokeMethod(m_handler, "stopSubscription",
                            Qt::QueuedConnection);
  QMetaObject::invokeMethod(m_handler, "subscriptionFailed",
                            Qt::QueuedConnection);
}
...
void IAPHandler::registerProducts(const QByteArray& data) {
  ...
  for (const Product& product : m_products) {
    productIdentifiers =
        [productIdentifiers setByAddingObject:product.m_name.toNSString()];
  }

  logger.log() << "We are about to register" << [productIdentifiers count]
               << "products";
...
void IAPHandler::productRegistered(void* a_product) {
  ...
  QString priceValue;
  {
    ...
    [numberFormatter
        setNumberStyle:(NSNumberFormatterStyle)NSNumberFormatterCurrencyStyle];
  ...
  NSDecimalNumber* monthlyPriceNS = nullptr;
  {
    ...
    if (mounthCount == 1) {
      ...
      NSDecimalNumber* divider =
          [[NSDecimalNumber alloc] initWithDouble:(double)mounthCount];
...
void IAPHandler::processCompletedTransactions(const QStringList& ids) {
  ...
  QString token = AndroidUtils::IAPToken();
  if (token.isEmpty()) {
    logger.log() << "Empty token found";
    emit subscriptionFailed();
    return;
  }

  NetworkRequest* request =
      NetworkRequest::createForAndroidPurchase(this, token);
